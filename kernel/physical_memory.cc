...
namespace memory
{
	...
	static uint32_t *memory_map_data = 0;
	...
	inline void
	mark_free_range( uint64_t addr, size_t len )
	{
	...
	void
	init_physical_memory( const multiboot_info_t *boot_info )
	{
		uint64_t mem_available = 0, first_free = 0, first_chunk_above_1mb = 0;
		...
		memory_map_size = mem_available / 4096 / 8;
		log::printk( "-----------------------------------------\n" );
		...
		do
		{
			...
			if( mem_map->type == MULTIBOOT_MEMORY_AVAILABLE && 
			    mem_map->addr >= first_free )
			{
				mark_free_range( mem_map->addr, mem_map->len );
			}
			mem_map = ( multiboot_memory_map_t* )( ( ptrdiff_t )mem_map + mem_map->size + sizeof( mem_map->size ) );
		} while( ( ptrdiff_t)mem_map < boot_info->mmap_addr + boot_info->mmap_length );
